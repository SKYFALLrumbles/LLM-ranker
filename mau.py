...
# Initialize session state for ranked resumes
...
if submit and uploaded_files:
    ...
    if not pdf_files:
        ...
        st.write(f"Processing {len(pdf_files)} PDFs.")

        # Read PDFs and process them
        pdf_contents = [pdf_file.read() for pdf_file in pdf_files]
            ...
            for idx, pdf_content in enumerate(pdf_contents):
                ...
                if response:
                    ...
                    if match_percentage is not None:
                        ...
                        resume_scores.append((response, match_percentage, pdf_files[idx].name, pdf_content, email))
                    else:
                        st.error(f"Could not extract match percentage from the response for {pdf_files[idx].name}.")
                else:
                    st.error(f"Failed to get a valid response for {pdf_files[idx].name}.")
